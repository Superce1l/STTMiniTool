...
- 🧩 **多推理核心**：CrispASR（GPU：CUDA／Vulkan 可选）／OpenVINO（纯 CPU）／chatllm（向下兼容），可于「模型」页切换
- ✂️ **长音频切片**：>2 小时自动 FFmpeg 切片转录，稳定不爆内存
...
| **模型下载位置** | Qwen/Shisper模型的存储目录；留空=程序目录下的ov_models(重启后生效)|
...
webview_backend.py      # WebView 后端逻辑（模型目录、加载、转录、设置）
webview/                # 前端（HTML / CSS / JS，i18n：简中 / English）
...
downloader.py           # 模型完整性检查与按需下载（含 HF 镜像、CUDA 版本推荐）
